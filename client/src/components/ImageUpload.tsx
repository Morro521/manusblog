...
export default function ImageUpload({ onUpload, value, maxSize = 5, label = "IMAGE FILE", compact = false }: ImageUploadProps) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!compact) setPreview(result.url);
      onUpload(result.url);
    } catch (uploadError) {
  ...
  return (
    <div>
      ...
      {compact ? <><button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploadMutation.isPending} className="inline-flex h-8 items-center gap-2 border border-white/[0.12] px-2.5 font-mono text-[10px] tracking-[0.08em] text-stone-500 transition-colors hover:border-[#c6edf0] hover:text-[#c6edf0] disabled:opacity-50">{uploadMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <Upload size={13} />}{uploadMutation.isPending ? "UPLOADING" : "INSERT IMAGE"}</button>{error && <p className="mt-2 border-l border-[#e39a86] pl-2 text-xs text-[#e39a86]">{error}</p>}</> : <>
