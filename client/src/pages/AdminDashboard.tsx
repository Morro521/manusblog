...
function Panel({ label, title, count, children }: { label: string; title: string; count?: number; children: React.ReactNode }) {
  return <section className="mt-6 rounded-[1.35rem] border border-white/[0.13] bg-[#202630]/64 p-5 shadow-lg shadow-black/10 sm:mt-8 sm:p-7"><div className="mb-6 flex items-end justify-between gap-4"><div><p className="text-sm text-slate-300">{label}</p><h2 className="display-title mt-2 text-3xl">{title}</h2></div>{count !== undefined && <span className="rounded-full bg-[#bce8eb]/14 px-3 py-1.5 text-sm text-[#d6fbfc]">{count} 条</span>}</div>{children}</section>;
}

function Empty({ title, description }: { title: string; description: string }) {
  return <div className="grid min-h-44 place-items-center rounded-2xl border border-dashed border-white/[0.16] bg-white/[0.025] px-5 text-center"><div><p className="text-sm font-medium text-slate-200">{title}</p><p className="mt-3 text-sm text-slate-400">{description}</p></div></div>;
}
...
export default function AdminDashboard() {
  ...
  if (loading) return <div className="grid min-h-[45vh] place-items-center"><p className="text-sm text-slate-300">正在验证管理员权限…</p></div>;
  if (user?.role !== "admin") return <div className="grid min-h-[55vh] place-items-center text-center"><div className="rounded-3xl border border-white/[0.14] bg-[#222832]/78 px-7 py-10 shadow-xl shadow-black/10"><p className="text-sm text-slate-300">这里仅供管理员使用</p><h1 className="display-title mt-4 text-4xl">你没有访问<br />管理页面的权限。</h1><Button onClick={() => navigate("/")} className="editorial-button mt-7 px-5">返回首页</Button></div></div>;

  return <div className="pb-10"><button onClick={() => navigate("/")} className="mb-6 flex items-center gap-2 rounded-lg px-2 py-2 text-sm text-slate-300 hover:bg-white/[0.08] hover:text-[#bce8eb]"><ChevronLeft size={15} /> 返回首页</button><section className="grid overflow-hidden rounded-[1.6rem] border border-white/[0.14] bg-[#202630]/78 px-6 py-10 shadow-xl shadow-black/10 sm:px-10 sm:py-14 lg:grid-cols-12"><div className="lg:col-span-7"><p className="text-sm text-slate-300">管理后台</p><h1 className="display-title mt-4 text-5xl sm:text-6xl">把内容，<br /><span className="display-accent">好好照看。</span></h1></div><div className="mt-7 max-w-md lg:col-span-4 lg:col-start-9 lg:mt-2"><p className="copy-lede">这里可以处理文章、评论、标签、分类、图片集与用户信息。每项操作完成后都会刷新对应数据。</p></div></section><Tabs value={selectedTab} onValueChange={setSelectedTab} className="mt-6"><TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-2xl border border-white/[0.13] bg-[#202630]/64 p-1.5"><TabsTrigger value="posts" className="console-tab rounded-xl"><FileText size={14} />文章</TabsTrigger><TabsTrigger value="comments" className="console-tab rounded-xl"><ShieldCheck size={14} />审核</TabsTrigger><TabsTrigger value="terms" className="console-tab rounded-xl">标签与分类</TabsTrigger><TabsTrigger value="gallery" className="console-tab rounded-xl"><ImageIcon size={14} />图片集</TabsTrigger><TabsTrigger value="users" className="console-tab rounded-xl"><Users size={14} />用户</TabsTrigger></TabsList><TabsContent value="posts" className="mt-0"><PostsManagement /></TabsContent><TabsContent value="comments" className="mt-0"><CommentsModeration /></TabsContent><TabsContent value="terms" className="mt-0"><TermsManagement /></TabsContent><TabsContent value="gallery" className="mt-0"><GalleryManagement /></TabsContent><TabsContent value="users" className="mt-0"><UsersDirectory /></TabsContent></Tabs></div>;
}
...
function GalleryManagement() {
  ...
  return <Panel label="图片管理" title="图片集" count={galleries?.length}><div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]"><aside className="rounded-2xl border border-white/[0.12] bg-white/[0.035] p-4"><p className="text-sm font-medium text-slate-100">创建图片集</p><Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="图片集名称" className="quiet-input mt-3 h-10 px-3 text-sm" /><textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="可选说明" className="quiet-input mt-3 min-h-20 w-full resize-y p-3 text-sm" /><Button disabled={createMutation.isPending || !title.trim()} onClick={() => createMutation.mutate({ title: title.trim(), description: description.trim() || undefined })} className="editorial-button editorial-button-primary mt-3 w-full">创建图片集</Button><div className="mt-7 border-t border-white/[0.12] pt-5"><p className="text-xs font-medium text-slate-300">已有图片集</p><div className="mt-3 grid gap-1.5">{galleries?.map((gallery: any) => <button key={gallery.id} onClick={() => setSelectedGalleryId(gallery.id)} className={`rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${selectedGalleryId === gallery.id ? "bg-[#bce8eb]/14 text-[#d6fbfc]" : "text-slate-200 hover:bg-white/[0.08]"}`}>{gallery.title}</button>)}</div></div></aside><div>{selectedGallery ? <div className="rounded-2xl border border-white/[0.12] bg-white/[0.035] p-4 sm:p-5"><div className="flex items-start justify-between gap-5"><div><p className="text-sm text-slate-300">正在编辑图片集</p><h3 className="mt-2 text-2xl font-medium text-white">{selectedGallery.title}</h3><p className="mt-2 text-sm leading-6 text-slate-300">{selectedGallery.description || "暂未添加说明。"}</p></div><Button disabled={deleteGalleryMutation.isPending} onClick={() => { if (window.confirm(`确定删除图片集「${selectedGallery.title}」及其图片记录吗？`)) deleteGalleryMutation.mutate({ id: selectedGallery.id }); }} variant="ghost" className="h-9 px-2.5 text-xs text-slate-300 hover:bg-[#efaa91]/10 hover:text-[#ffd1c4]"><Trash2 size={13} className="mr-1.5" />删除</Button></div><div className="mt-6 rounded-2xl border border-white/[0.12] bg-[#15191f]/48 p-4"><ImageUpload onUpload={addUploadedImage} label="上传图片到这个图片集" /></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{selectedGallery.images.length ? selectedGallery.images.map((image: any, index: number) => <div key={image.id} className="overflow-hidden rounded-xl border border-white/[0.12] bg-[#15191f]/48"><img src={image.url} alt={image.title || `图片 ${index + 1}`} className="aspect-[1.4] w-full object-cover" /><div className="flex items-center justify-between p-3"><span className="text-xs text-slate-300">图片 {index + 1}</span><Button disabled={removeImageMutation.isPending} onClick={() => removeImageMutation.mutate({ id: image.id })} variant="ghost" className="h-8 px-2 text-xs text-slate-300 hover:bg-[#efaa91]/10 hover:text-[#ffd1c4]"><Trash2 size={13} /></Button></div></div>) : <div className="sm:col-span-2"><Empty title="这个图片集还没有图片" description="上传真实图片后会显示在这里。" /></div>}</div></div> : <Empty title="选择一个图片集" description="从左侧选择，或先创建新的图片集。" />}</div></div></Panel>;
}
