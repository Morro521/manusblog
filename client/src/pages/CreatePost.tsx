...
export default function CreatePost() {
  ...
  const publish = () => createMutation.mutate({ title: title.trim(), slug: slug.trim(), content, excerpt: excerpt.trim() || undefined, coverImage: coverImage.trim() || undefined, categoryId, tagIds, status: "published" });
  ...
  return <div className="pb-16"><header className="sticky top-0 z-30 -mx-4 border-b border-white/[0.12] bg-[#10110f]/95 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:px-6"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><button onClick={() => navigate("/posts")} className="grid h-8 w-8 shrink-0 place-items-center border border-white/[0.12] text-stone-500 hover:border-[#c6edf0] hover:text-[#c6edf0]" aria-label="返回文章索引"><ChevronLeft size={15} /></button><div className="min-w-0"><p className="truncate font-mono text-[10px] tracking-[0.1em] text-stone-400">{title || "UNTITLED ENTRY"}</p><p className={`mt-0.5 font-mono text-[9px] tracking-[0.08em] ${dirty ? "text-[#e2b27c]" : "text-stone-600"}`}>{dirty ? "UNPUBLISHED" : "READY"} · {wordCount} WORDS</p></div></div><div className="flex items-center gap-2"><Button type="button" onClick={() => setSettingsOpen(true)} variant="ghost" className="h-8 gap-1.5 px-2 text-xs text-stone-400 hover:bg-white/[0.04] hover:text-[#c6edf0]"><Settings2 size={14} /><span className="hidden sm:inline">文档设置</span></Button><Button disabled={createMutation.isPending || !canSubmit} onClick={publish} className="editorial-button editorial-button-primary h-8 px-3 text-xs">{createMutation.isPending ? "处理中…" : <><Send size={13} className="mr-1.5" />发布</>}</Button></div></div></header>

    <main className="mx-auto max-w-[820px] pt-12 sm:pt-16"><div className="border-b border-white/[0.12] pb-7"><p className="editorial-kicker">NEW TRANSMISSION / ENTRY 01</p><Input value={title} onChange={(event) => changeTitle(event.target.value)} placeholder="无标题记录" className="mt-4 h-auto border-0 bg-transparent px-0 py-0 font-serif text-4xl leading-tight text-stone-100 placeholder:text-stone-700 focus-visible:ring-0 sm:text-6xl" /><p className="mt-4 text-sm leading-7 text-stone-600">先写下你真正想说的内容。分类、封面、摘要和链接都在右上角的文档设置中。</p></div><section className="pt-2"><MarkdownEditor value={content} onChange={setContent} placeholder="从这里开始。先把真正的问题放在页面上。" /></section>{createMutation.error && <p className="mt-5 border-l border-[#e39a86] pl-3 text-sm text-[#e39a86]">{createMutation.error.message}</p>}<footer className="mt-8 flex flex-col justify-between gap-4 border-y border-white/[0.12] py-4 sm:flex-row sm:items-center"><p className="text-xs leading-5 text-stone-600">填写完成后发布，文章会出现在公开文章索引。</p><div className="flex sm:hidden"><Button disabled={createMutation.isPending || !canSubmit} onClick={publish} className="editorial-button editorial-button-primary px-3 text-xs"><Send size={13} className="mr-1" />发布记录</Button></div></footer></main>
