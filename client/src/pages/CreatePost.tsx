  ...
  return (
    <div className="pb-10">
      ...
      <section className="grid gap-10 pt-10 lg:grid-cols-12 lg:pt-14">
        <aside className="border-b border-white/[0.12] pb-7 lg:col-span-3 lg:border-b-0 lg:border-r lg:pr-8 lg:pb-0">
          ...
          <div className="mt-8 border-t border-white/[0.12] pt-5"><ImageUpload value={coverImage} onUpload={setCoverImage} label="COVER IMAGE" /><label className="mt-5 block text-xs leading-5 text-stone-600">也可直接粘贴已有的公开或 `/manus-storage/` 图片 URL。</label><Input value={coverImage} onChange={(event) => setCoverImage(event.target.value)} placeholder="https://…" className="quiet-input mt-2 h-10 border-x-0 border-t-0 px-0 text-sm placeholder:text-stone-700" /></div>
        </aside>
