...
export default function CreatePost() {
  ...
  const persist = (status: "draft" | "published") => {
    if (isEditing) { updateMutation.mutate({ id: editId, title: title.trim(), slug: slug.trim(), content, excerpt: excerpt.trim() || null, coverImage: coverImage.trim() || null, categoryId: categoryId ?? null, tagIds, status }, { onSuccess: () => navigate(status === "draft" ? "/workspace" : `/posts/${slug}`) }); return; }
  ...
  return <div className="pb-16"><header className="sticky top-3 z-30 mx-auto max-w-6xl border-y border-white/[0.28] bg-[#3a4a54]/84 px-3 py-2 backdrop-blur-xl sm:px-4"><div className="mx-auto flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><button onClick={() => navigate("/posts")} className="grid h-9 w-9 shrink-0 place-items-center border border-white/[0.28] text-[#f0ede7] hover:border-[#d0f4ee] hover:text-[#d0f4ee]" aria-label="返回文章列表"><ChevronLeft size={16} /></button><div className="min-w-0"><p className="truncate text-xs font-medium text-[#fff8ed]">{title || (isEditing ? "正在编辑文章" : "未命名文章")}</p><p className={`mt-0.5 text-[11px] ${dirty ? "text-[#fff0df]" : "text-[#d0cfca]"}`}>{dirty ? "有未保存的修改" : isEditing ? (editablePost?.status === "published" ? "已发布，暂无新修改" : "草稿已保存") : "尚未保存"} · 正文 {wordCount} 字</p></div></div><div className="flex items-center gap-1.5"><Button type="button" onClick={() => setSettingsOpen(true)} variant="ghost" className="h-9 gap-1.5 px-2.5 text-xs text-[#f0ede7] hover:bg-white/[0.08] hover:text-[#d0f4ee]"><Settings2 size={14} /><span className="hidden sm:inline">设置</span></Button>{canSaveDraft && <Button disabled={isSubmitting || !canSubmit} onClick={() => persist("draft")} variant="ghost" className="hidden h-9 px-2.5 text-xs text-[#fff8ed] hover:bg-white/[0.08] sm:inline-flex"><Save size={13} className="mr-1.5" />保存草稿</Button>}<Button disabled={isSubmitting || !canSubmit} onClick={() => persist("published")} className="editorial-button editorial-button-primary h-9 px-3.5 text-xs">{isSubmitting ? "正在处理…" : <><Send size={13} className="mr-1.5" />{isEditing ? "更新文章" : "发布文章"}</>}</Button></div></div></header>

    <main className="mx-auto max-w-[860px] pt-12 sm:pt-16"><div className="border-l border-[#eab78c] px-5 py-3 sm:px-8"><p className="text-sm text-[#e5e2db]">{isEditing ? "编辑文章" : "新建文章"}</p><Input value={title} onChange={(event) => changeTitle(event.target.value)} placeholder="给这篇文章起一个明确的标题" className="mt-4 h-auto border-0 bg-transparent px-0 py-0 font-serif text-4xl leading-tight text-[#fff8ed] placeholder:text-[#c4c6c2] focus-visible:ring-0 sm:text-6xl" /><p className="mt-5 max-w-xl text-sm leading-7 text-[#e5e2db]">先写正文；分类、标签、封面、摘要和链接可在右上角的设置中补充。</p></div><section className="border-t border-white/[0.2] pt-5"><MarkdownEditor value={content} onChange={setContent} placeholder="从这里开始：写下问题、过程、依据和暂时的结论。" /></section>{(createMutation.error || updateMutation.error) && <p className="mt-5 border-l-2 border-[#eab78c] bg-[#eab78c]/15 px-4 py-3 text-sm text-[#fff0df]">{createMutation.error?.message || updateMutation.error?.message}</p>}<footer className="mt-8 flex flex-col justify-between gap-4 border-y border-white/[0.2] px-1 py-4 sm:flex-row sm:items-center"><p className="text-xs leading-5 text-[#d0cfca]">{isEditing ? "更新后会返回这篇文章的公开页面。" : "草稿仅保存在你的工作区；发布后才会出现在公开文章列表。"}</p><div className="flex gap-2 sm:hidden">{canSaveDraft && <Button disabled={isSubmitting || !canSubmit} onClick={() => persist("draft")} variant="ghost" className="editorial-button px-3 text-xs"><Save size={13} className="mr-1" />保存草稿</Button>}<Button disabled={isSubmitting || !canSubmit} onClick={() => persist("published")} className="editorial-button editorial-button-primary px-3 text-xs"><Send size={13} className="mr-1" />{isEditing ? "更新文章" : "发布文章"}</Button></div></footer></main>
