...
export default function GalleryDetail() {
  ...
  const { data: gallery, isLoading } = trpc.galleries.getById.useQuery({ id: galleryId }, { enabled: galleryId > 0 });

  if (isLoading) return <div className="grid min-h-[55vh] place-items-center"><p className="editorial-kicker">OPENING CABINET…</p></div>;
  if (!gallery) return <div className="grid min-h-[55vh] place-items-center text-center"><div><p className="editorial-kicker">CABINET NOT FOUND</p><h1 className="display-title mt-4 text-4xl">这个图片集不在当前索引里。</h1><button onClick={() => navigate("/gallery")} className="editorial-button mt-7 px-5">返回图片集</button></div></div>;
