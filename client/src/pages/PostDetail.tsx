  ...
  return (
    <article className="pb-8">
      ...
      <div className="grid gap-10 pt-10 lg:grid-cols-12 lg:pt-14">
        <aside className="border-b border-white/[0.12] pb-6 lg:col-span-3 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-8"><div className="grid gap-6 sm:grid-cols-3 lg:grid-cols-1"><div><p className="editorial-kicker">AUTHOR</p><p className="mt-2 flex items-center gap-2 text-sm text-stone-300"><UserRound size={14} className="text-[#c6edf0]" /> Observer #{post.authorId}</p>{canEditPost && <Button onClick={() => navigate(`/edit/${post.id}`)} variant="ghost" className="mt-4 h-8 px-0 text-xs text-[#c6edf0] hover:bg-transparent hover:text-stone-100"><Pencil size={13} className="mr-1.5" />编辑这篇记录</Button>}</div><div><p className="editorial-kicker">TAGS</p><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">{post.tags.length ? post.tags.map((tag) => <button onClick={() => navigate(`/posts?tag=${tag.slug}`)} key={tag.id} className="text-xs text-stone-500 hover:text-[#c6edf0]">#{tag.name}</button>) : <span className="text-xs text-stone-600">—</span>}</div></div><div><p className="editorial-kicker">READING NOTE</p><p className="mt-2 text-xs leading-6 text-stone-500">建议在安静的时间读完。正文内的链接均会在新窗口中打开。</p></div></div></aside>
        <section className="min-w-0 lg:col-span-7 lg:col-start-5"><div className="prose-cosmic"><ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]} components={{ h1: ({ node, ...props }) => <h1 className="mb-6 mt-12 text-3xl" {...props} />, h2: ({ node, ...props }) => <h2 className="mb-5 text-2xl" {...props} />, h3: ({ node, ...props }) => <h3 className="mb-4 mt-9 text-xl" {...props} />, p: ({ node, ...props }) => <p className="mb-7" {...props} />, code: ({ node, inline, children, ...props }: any) => inline ? <code className="bg-white/[0.06] px-1.5 py-0.5 text-sm" {...props}>{children}</code> : <code {...props}>{children}</code>, pre: ({ node, ...props }) => <pre className="mb-8 overflow-x-auto p-5 text-sm leading-7" {...props} />, blockquote: ({ node, ...props }) => <blockquote className="mb-8 italic" {...props} />, ul: ({ node, ...props }) => <ul className="mb-7 list-disc space-y-2 pl-6" {...props} />, ol: ({ node, ...props }) => <ol className="mb-7 list-decimal space-y-2 pl-6" {...props} />, table: ({ node, ...props }) => <div className="mb-8 overflow-x-auto"><table className="w-full border-collapse text-sm" {...props} /></div>, th: ({ node, ...props }) => <th className="border border-white/15 bg-white/[0.04] p-3 text-left font-sans text-[#c6edf0]" {...props} />, td: ({ node, ...props }) => <td className="border border-white/10 p-3" {...props} />, a: ({ node, ...props }) => <a target="_blank" rel="noreferrer" {...props} />, img: ({ node, ...props }) => <img className="my-9 w-full border border-white/10" loading="lazy" {...props} /> }}>{post.content}</ReactMarkdown></div><div className="mt-12 flex justify-end border-t border-white/[0.12] pt-5"><button onClick={() => navigate("/posts")} className="group flex items-center gap-2 text-sm text-stone-500 hover:text-[#c6edf0]">回到索引 <ArrowUpRight size={14} className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" /></button></div></section>
      </div>

      <section className="mx-auto mt-20 max-w-4xl border-t border-white/[0.15] pt-8 sm:mt-28"><div className="flex items-end justify-between"><div><p className="editorial-kicker">RESPONSE LOG</p><h2 className="display-title mt-3 text-3xl">讨论与回声</h2></div><span className="article-index">{String(commentCount).padStart(2, "0")} NOTES</span></div>{isAuthenticated ? <div className="mt-8 border-y border-white/[0.12] py-5"><textarea value={commentContent} onChange={(event) => setCommentContent(event.target.value)} placeholder="留下你的想法。新评论将在审核后公开。" className="quiet-input min-h-28 w-full resize-y p-4 text-sm leading-6 placeholder:text-stone-600" /><div className="mt-3 flex items-center justify-between gap-4"><p className="text-xs text-stone-600">你的待审核评论仅对你可见，审核通过后才会公开。</p><Button disabled={createCommentMutation.isPending || !commentContent.trim()} onClick={() => createCommentMutation.mutate({ postId: post.id, content: commentContent })} className="editorial-button editorial-button-primary shrink-0 px-4">{createCommentMutation.isPending ? "发送中" : <><Send size={13} className="mr-1.5" /> 发布回应</>}</Button></div></div> : <div className="mt-8 border-y border-white/[0.12] py-8 text-center"><p className="text-sm text-stone-500">登录后，可以为这篇文章留下回应。</p><button onClick={() => navigate("/")} className="mt-3 text-sm text-[#c6edf0]">回到首页登录</button></div>}{createCommentMutation.error && <p className="mt-4 border-l border-[#e39a86] pl-3 text-sm text-[#e39a86]">{createCommentMutation.error.message}</p>}{deleteCommentMutation.error && <p className="mt-4 border-l border-[#e39a86] pl-3 text-sm text-[#e39a86]">{deleteCommentMutation.error.message}</p>}<div>{comments?.data?.length ? comments.data.map((comment: any, index: number) => renderComment(comment, String(index + 1).padStart(2, "0"))) : <div className="border-b border-white/[0.12] py-12 text-center"><p className="editorial-kicker">NO RESPONSE YET</p><p className="mt-3 text-sm text-stone-500">第一段回应，将从这里开始。</p></div>}</div></section>
    </article>
  );
}
