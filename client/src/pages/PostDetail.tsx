...
export default function PostDetail() {
  ...
  const createCommentMutation = trpc.comments.create.useMutation({ onSuccess: () => { setCommentContent(""); setReplyContent(""); setReplyTarget(null); void refetchComments(); } });
  ...
  const renderComment = (comment: any, index: string, isReply = false) => (
    <article key={comment.id} className={`${isReply ? "ml-4 border-l border-white/[0.22] pl-4 sm:ml-8 sm:pl-6" : ""} grid gap-3 border-b border-white/[0.2] py-6 sm:grid-cols-[110px_1fr] sm:gap-6`}>
      <div><p className="article-index">{index} / {isReply ? "回复" : "评论"}</p><p className="mt-2 text-xs text-[#d0cfca]">作者 #{comment.authorId}</p><p className="mt-1 text-[10px] text-[#b8b9b5]">{new Date(comment.createdAt).toLocaleDateString()}</p>{comment.status === "pending" && <span className="mt-3 inline-block border border-[#eab78c]/70 px-1.5 py-1 font-mono text-[9px] tracking-[0.1em] text-[#fff0df]">等待审核</span>}</div>
      <div><p className="text-sm leading-7 text-[#f0ede7]">{comment.content}</p><div className="mt-4 flex flex-wrap items-center gap-4"><button onClick={() => { setReplyTarget(replyTarget === comment.id ? null : comment.id); setReplyContent(""); }} className="flex items-center gap-1.5 border-b border-transparent pb-1 text-xs text-[#d0cfca] hover:border-[#d0f4ee] hover:text-[#d0f4ee]"><CornerDownRight size={13} />回复</button>{canDelete(comment) && <button disabled={deleteCommentMutation.isPending} onClick={() => deleteCommentMutation.mutate({ id: comment.id })} className="flex items-center gap-1.5 border-b border-transparent pb-1 text-xs text-[#d0cfca] hover:border-[#eab78c] hover:text-[#fff0df]"><Trash2 size={12} />删除</button>}</div>{replyTarget === comment.id && <div className="mt-5 border-l border-[#d0f4ee] pl-4"><div className="mb-2 flex items-center justify-between"><p className="editorial-kicker">回复作者 #{comment.authorId}</p><button onClick={() => setReplyTarget(null)} className="text-[#d0cfca] hover:text-white" aria-label="取消回复"><X size={14} /></button></div><textarea value={replyContent} onChange={(event) => setReplyContent(event.target.value)} placeholder="写下你的回复。" className="quiet-input min-h-20 w-full resize-y p-3 text-sm leading-6" /><div className="mt-3 flex justify-end"><Button disabled={createCommentMutation.isPending || !replyContent.trim()} onClick={() => submitReply(comment.id)} className="editorial-button editorial-button-primary px-3">发送回复</Button></div></div>}{Array.isArray(comment.replies) && comment.replies.map((reply: any, replyIndex: number) => renderComment(reply, `${index}.${replyIndex + 1}`, true))}</div>
    </article>
  );

  return (
    <article className="pb-16">
      <button onClick={() => navigate("/posts")} className="group mb-7 flex items-center gap-2 border-b border-transparent pb-1 text-sm text-[#e5e2db] transition-colors hover:border-[#d0f4ee] hover:text-[#d0f4ee]"><ArrowLeft size={15} />回到文章列表</button>
      ...
      <section className="mx-auto mt-16 max-w-4xl border-t border-white/[0.28] pt-8 sm:mt-24 sm:pt-10"><div className="flex items-end justify-between"><div><p className="editorial-kicker">评论</p><h2 className="display-title mt-3 text-3xl">参与讨论。</h2></div><span className="text-sm text-[#d5d5d0]">{commentCount} 条评论</span></div>{isAuthenticated ? <div className="mt-8 border-y border-white/[0.2] py-4"><textarea value={commentContent} onChange={(event) => setCommentContent(event.target.value)} placeholder="补充你的看法。审核通过后，评论会显示在文章下方。" className="quiet-input min-h-28 w-full resize-y p-4 text-sm leading-6" /><div className="mt-3 flex items-center justify-between gap-4"><p className="text-xs text-[#d0cfca]">提交后只有你能看到，直到审核完成。</p><Button disabled={createCommentMutation.isPending || !commentContent.trim()} onClick={() => createCommentMutation.mutate({ postId: post.id, content: commentContent })} className="editorial-button editorial-button-primary shrink-0 px-4">{createCommentMutation.isPending ? "正在提交…" : <><Send size={13} className="mr-1.5" />提交评论</>}</Button></div></div> : <div className="mt-8 border-y border-white/[0.2] px-2 py-8 text-center"><p className="text-sm text-[#f0ede7]">登录后才能发表或回复评论。</p><button onClick={() => navigate("/")} className="mt-3 border-b border-[#d0f4ee] pb-1 text-sm text-[#d0f4ee] hover:text-white">返回首页登录</button></div>}{createCommentMutation.error && <p className="mt-4 border-l-2 border-[#eab78c] bg-[#eab78c]/15 px-4 py-3 text-sm text-[#fff0df]">{createCommentMutation.error.message}</p>}{deleteCommentMutation.error && <p className="mt-4 border-l-2 border-[#eab78c] bg-[#eab78c]/15 px-4 py-3 text-sm text-[#fff0df]">{deleteCommentMutation.error.message}</p>}<div className="mt-5 border-t border-white/[0.2]">{comments?.data?.length ? comments.data.map((comment: any, index: number) => renderComment(comment, String(index + 1).padStart(2, "0"))) : <div className="py-12 text-center"><p className="text-sm font-medium text-[#f0ede7]">还没有人留言</p><p className="mt-3 text-sm text-[#d0cfca]">如果有补充或不同看法，可以从上方开始。</p></div>}</div></section>
    </article>
  );
}
