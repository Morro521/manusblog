...
describe("posts.list server-side filters", () => {
  ...
  it("passes a tag filter and page-two offset to both data and total queries", async () => {
    ...
    expect(dbMocks.getPublishedPosts).toHaveBeenCalledWith(10, 10, { tagSlug: "hardware", categoryId: undefined });
    expect(dbMocks.getPostCount).toHaveBeenCalledWith("published", { tagSlug: "hardware", categoryId: undefined });
  ...
  it("passes a category filter to the server instead of filtering only the loaded client page", async () => {
    ...
    expect(dbMocks.getPublishedPosts).toHaveBeenCalledWith(5, 10, { tagSlug: undefined, categoryId: 7 });
    expect(dbMocks.getPostCount).toHaveBeenCalledWith("published", { tagSlug: undefined, categoryId: 7 });
  ...
  it("keeps tag and category constraints together for a paginated combined filter", async () => {
    ...
    const filters = { tagSlug: "hardware", categoryId: 3 };
